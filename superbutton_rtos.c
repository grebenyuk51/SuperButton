...
void process_button_events_after_interrupt(void* pvParameters) 
{
    ...
    while(true)
    {
        ...
        for(int i = args.button_info_index; i < button_info_len + args.button_info_index; i++)
        {
            ...
            if(current_period > (SUPER_BUTTON_DEBOUNCE_MS / portTICK_PERIOD_MS))
            {
                ...
                if(current_period >= (SUPER_BUTTON_MULTI_CLICK_GAP_MS / portTICK_PERIOD_MS) && current_period < (SUPER_BUTTON_LONG_PRESS_START_GAP_MS / portTICK_PERIOD_MS))
                {
                ...
                else if(current_period >= (SUPER_BUTTON_LONG_PRESS_START_GAP_MS / portTICK_PERIOD_MS))
                {
        ...
        if(isbusy)
        {
            queue_receive_delay = 10 / portTICK_PERIOD_MS;
        }
        else
        {
